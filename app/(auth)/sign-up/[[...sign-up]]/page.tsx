...
export default function SignUpPage() {
	...
	const globalErrors = errors.global?.map((error) => (
		<span
			key={error.message}
			className="block text-sm text-red-400 bg-red-900/20 p-3 rounded-lg border border-red-800"
		>
			{error.message}
		</span>
	));
	...
	return (
		<div className="min-h-screen flex items-center justify-center bg-gray-900 p-4">
			<div className="w-full max-w-sm mx-auto bg-gray-800/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-700/50 p-8 space-y-6">
				<header className="text-center">
					<div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-6">
						<MessageCircle className="w-8 h-8 text-white" />
					</div>
